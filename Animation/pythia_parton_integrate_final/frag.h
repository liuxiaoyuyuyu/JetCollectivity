...
stringFragmentation::stringFragmentation()
    :Pass_flag(0),Nchcut(100),JetPtCut(550),JetEtaCut(0.5),JetPt(0),JetEta(0),JetPhi(0),JetNch(0)
{
...
int stringFragmentation::initPythia(){
...
void stringFragmentation::JetFinder(){
    Pythia8::ParticleData &particleData = pythia.particleData;
    const double R_jet = 0.8; // CMS cut, CMS PAS HIN-21-013           
    input_particles.clear();
    Pass_flag=0;   
    for (int i = 0; i < pythia.event.size(); i++) {
        fastjet::PseudoJet particlefastjet = PseudoJet(pythia.event[i].px(), pythia.event[i].py(), pythia.event[i].pz(), pythia.event[i].e());
    ...
    for (unsigned int i = 0; i < inclusive_jets.size(); i++) {
        if (   ( inclusive_jets[i].pt() > JetPtCut ) && ( abs(inclusive_jets[i].eta()) <JetEtaCut   )    ) {//jetPtCut=550,JetEtaCut=1.6
            vector<fastjet::PseudoJet> constituents = inclusive_jets[i].constituents();
             int Nch = 0; 

            for(int i_cons=0;i_cons < constituents.size();i_cons++ ){
                int tmp_pid=constituents[i_cons].user_index();

                if( particleData.charge(tmp_pid)==0) continue;
                if( constituents[i_cons].pt()<0.3  ) continue;
                if( abs(constituents[i_cons].eta())>2.4 ) continue;

                 Nch++;//The charged multiplicity

            }

            if( Nch>Nchcut ) { Pass_flag++;JETindex=i;JetNch=Nch;}
    ...
    JetPt=inclusive_jets[JETindex].pt();
    JetEta=inclusive_jets[JETindex].eta();
    JetPhi=inclusive_jets[JETindex].phi();
    JetRapidity=inclusive_jets[JETindex].rapidity();
...
void stringFragmentation::PrintResult(){
    std::cout<<"There are "<<Pass_flag<<" Jets Pass JetCut:"<<"JetPt > "<<JetPtCut<<" "<<"abs(JetEta)<"<<JetEtaCut<<std::endl;
    //for(int ijet=0;ijet<inclusive_jets.size();ijet++ ){
    //    std::cout<<"Jet"<<ijet<<":"<<"Px="<<inclusive_jets[ijet].px()<<" "<<"Py="<<inclusive_jets[ijet].py()<<" "<<"Pz="<<inclusive_jets[ijet].pz()<<" "<<"E="<<inclusive_jets[ijet].e()<<std::endl;

    //}
    std::cout<<"JetPt="<<JetPt<<" "<<"JetEta="<<JetEta<<" "<<"JetPhi="<<JetPhi<<" "<<"JetNch="<<JetNch<<std::endl;
